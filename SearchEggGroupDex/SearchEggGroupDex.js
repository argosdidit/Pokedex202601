  ...
  func = {
    ...
    makeDecisionButton: function(){
      if(flag){
        ...
        <button id="BtnReload">
        <a id="decisionButton" href="../SearchEggGroupDex/SearchEggGroupDex.html">
      ...
      if(flag){
        switch(jap_or_eng)
        {
          case 'JAP':
            ...
            .then(res => res.json())
            .then(egg_groups => func.renderJapEggGroupTable(egg_groups));
            break;
          case 'ENG':
            //fetch('http://127.0.0.1:3001/api/poke/egg_group')
            fetch('/api/poke/egg_group')
            .then(res => res.json())
            .then(egg_groups => func.renderEngEggGroupTable(egg_groups));
            break;
          default:
            //fetch('http://127.0.0.1:3001/api/poke/egg_group')
            fetch('/api/poke/egg_group')
    ...
    renderJapEggGroupTable: function(egg_groups){
      if(flag){
    ...
    renderEngEggGroupTable: function(egg_groups){
      if(flag){
        const area = document.getElementById("TblSearchEggGroup");
        area.innerHTML = "";
      ...
      if(flag){
        ...
        switch(bright_or_dark)
        {
          case 'BRIGHT':
            htmlPageStyle =
            `
            <link rel="stylesheet" href="SearchEggGroupDexBright.css">
            <link rel="icon" href="SearchEggGroupDexBright.png">
            `;
            break;
          case 'DARK':
            htmlPageStyle =
            `
            <link rel="stylesheet" href="SearchEggGroupDexDark.css">
            <link rel="icon" href="SearchEggGroupDexDark.png">
            `;
            break;
          default:
            htmlPageStyle =
